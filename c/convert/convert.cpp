...
bool LoadDataStructures(char *filename, int vertexCount, int textureCount, int normalCount, int faceCount) {
    ...
    while (!fin.eof()) {
        ...
        if (input == 'f') {
            fin.get(input);
            if (input == ' ') {
                // Read the face data in reverse order to convert it to a left hand system from Blender's coordinate
                // system.
                fin >> faces[faceIndex].vIndex3 >> input2 >> faces[faceIndex].tIndex3 >> input2 >>
                    faces[faceIndex].nIndex3 >> faces[faceIndex].vIndex2 >> input2 >> faces[faceIndex].tIndex2 >>
                    input2 >> faces[faceIndex].nIndex2 >> faces[faceIndex].vIndex1 >> input2 >>
