...
DepthShaderClass::DepthShaderClass() { m_OpenGLPtr = 0; }
...
bool DepthShaderClass::Initialize(OpenGLClass *OpenGL) {
  ...
  strcpy(vsFilename, "./shaders/depth.vs");
  strcpy(psFilename, "./shaders/depth.ps");
...
void DepthShaderClass::OutputShaderErrorMessage(unsigned int shaderId,
                                                char *shaderFilename) {
  long count;
  int logSize, error;
  char *infoLog;
...
bool DepthShaderClass::SetShaderParameters(float *worldMatrix,
                                           float *viewMatrix,
