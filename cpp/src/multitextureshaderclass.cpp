...
MultiTextureShaderClass::MultiTextureShaderClass() { m_OpenGLPtr = 0; }

MultiTextureShaderClass::~MultiTextureShaderClass() {}

MultiTextureShaderClass::MultiTextureShaderClass(
    const MultiTextureShaderClass &other) {}
...
bool MultiTextureShaderClass::Initialize(OpenGLClass *OpenGL) {
  ...
  strcpy(vsFilename, "./shaders/multitexture.vs");
  strcpy(psFilename, "./shaders/multitexture.ps");
...
bool MultiTextureShaderClass::InitializeShader(char *vsFilename,
                                               char *fsFilename) {
...
void MultiTextureShaderClass::OutputShaderErrorMessage(unsigned int shaderId,
                                                       char *shaderFilename) {
  long count;
  int logSize, error;
  char *infoLog;
...
bool MultiTextureShaderClass::SetShaderParameters(float *worldMatrix,
                                                  float *viewMatrix,
  ...
  // Set the first texture in the pixel shader to use the data from the first
  // texture unit.
  location =
      m_OpenGLPtr->glGetUniformLocation(m_shaderProgram, "shaderTexture1");
  if (location == -1) {
    return false;
  }
  m_OpenGLPtr->glUniform1i(location, 0);

  // Set the second texture in the pixel shader to use the data from the second
  // texture unit.
  location =
      m_OpenGLPtr->glGetUniformLocation(m_shaderProgram, "shaderTexture2");
  if (location == -1) {
    return false;
  }
