...
WaterShaderClass::WaterShaderClass() { m_OpenGLPtr = 0; }
...
bool WaterShaderClass::Initialize(OpenGLClass *OpenGL) {
  ...
  strcpy(vsFilename, "./shaders/water.vs");
  strcpy(psFilename, "./shaders/water.ps");
...
void WaterShaderClass::OutputShaderErrorMessage(unsigned int shaderId,
                                                char *shaderFilename) {
  long count;
  int logSize, error;
  char *infoLog;
...
bool WaterShaderClass::SetShaderParameters(float *worldMatrix,
                                           float *viewMatrix,
  ...
  location =
      m_OpenGLPtr->glGetUniformLocation(m_shaderProgram, "reflectionMatrix");
  if (location == -1) {
    return false;
  }
  ...
  location =
      m_OpenGLPtr->glGetUniformLocation(m_shaderProgram, "normalTexture");
  if (location == -1) {
    return false;
  }
  ...
  location =
      m_OpenGLPtr->glGetUniformLocation(m_shaderProgram, "refractionTexture");
  if (location == -1) {
    return false;
  }
  ...
  location =
      m_OpenGLPtr->glGetUniformLocation(m_shaderProgram, "reflectionTexture");
  if (location == -1) {
    return false;
  }
  ...
  location =
      m_OpenGLPtr->glGetUniformLocation(m_shaderProgram, "waterTranslation");
  if (location == -1) {
    return false;
  }
  ...
  location =
      m_OpenGLPtr->glGetUniformLocation(m_shaderProgram, "reflectRefractScale");
  if (location == -1) {
    return false;
  }
