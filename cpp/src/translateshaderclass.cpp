...
TranslateShaderClass::TranslateShaderClass() { m_OpenGLPtr = 0; }
...
bool TranslateShaderClass::Initialize(OpenGLClass *OpenGL) {
  ...
  strcpy(vsFilename, "./shaders/translate.vs");
  strcpy(psFilename, "./shaders/translate.ps");
...
bool TranslateShaderClass::InitializeShader(char *vsFilename,
                                            char *fsFilename) {
...
void TranslateShaderClass::OutputShaderErrorMessage(unsigned int shaderId,
                                                    char *shaderFilename) {
  long count;
  int logSize, error;
  char *infoLog;
...
bool TranslateShaderClass::SetShaderParameters(float *worldMatrix,
                                               float *viewMatrix,
  ...
  location =
      m_OpenGLPtr->glGetUniformLocation(m_shaderProgram, "textureTranslation");
  if (location == -1) {
    return false;
  }
