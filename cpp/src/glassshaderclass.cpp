...
GlassShaderClass::GlassShaderClass() { m_OpenGLPtr = 0; }
...
bool GlassShaderClass::Initialize(OpenGLClass *OpenGL) {
  ...
  strcpy(vsFilename, "./shaders/glass.vs");
  strcpy(psFilename, "./shaders/glass.ps");
...
void GlassShaderClass::OutputShaderErrorMessage(unsigned int shaderId,
                                                char *shaderFilename) {
  long count;
  int logSize, error;
  char *infoLog;
...
bool GlassShaderClass::SetShaderParameters(float *worldMatrix,
                                           float *viewMatrix,
  ...
  location =
      m_OpenGLPtr->glGetUniformLocation(m_shaderProgram, "refractionScale");
  if (location == -1) {
    return false;
  }
  m_OpenGLPtr->glUniform1i(location, refractionScale);
