...
bool ColorShaderClass::Initialize(OpenGLClass *OpenGL) {
  ...
  strcpy(vsFilename, "./shaders/color.vs");
  strcpy(psFilename, "./shaders/color.ps");
...
bool ColorShaderClass::InitializeShader(char *vsFilename, char *fsFilename) {
  ...
  if (!vertexShaderBuffer) {
    cout << "Unable to open vertex shader at " << vsFilename << "\n";
    return false;
  }

  fragmentShaderBuffer = LoadShaderSourceFile(fsFilename);
  if (!fragmentShaderBuffer) {
    cout << "Unable to open fragment shader at " << vsFilename << "\n";
    return false;
  }
...
void ColorShaderClass::OutputShaderErrorMessage(unsigned int shaderId,
  ...
  if (filePtr == NULL) {
    cout << "ERROR: error opening shader error message output file." << endl;
    return;
  }

  count = fwrite(infoLog, sizeof(char), logSize, filePtr);
  if (count != logSize) {
    cout << "ERROR: error writing shader error message output file." << endl;
    return;
  }

  error = fclose(filePtr);
  if (error != 0) {
    cout << "ERROR: error closing shader error message output file." << endl;
    return;
  }

  cout << "Error compiling shader. Check shader-error.txt for the message. "
          "Shader filename: "
       << shaderFilename << endl;
