...
FadeShaderClass::FadeShaderClass() { m_OpenGLPtr = 0; }
...
bool FadeShaderClass::Initialize(OpenGLClass *OpenGL) {
  ...
  strcpy(vsFilename, "./shaders/fade.vs");
  strcpy(psFilename, "./shaders/fade.ps");
...
void FadeShaderClass::OutputShaderErrorMessage(unsigned int shaderId,
                                               char *shaderFilename) {
  long count;
  int logSize, error;
  char *infoLog;
...
bool FadeShaderClass::SetShaderParameters(float *worldMatrix, float *viewMatrix,
