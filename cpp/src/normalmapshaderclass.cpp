...
NormalMapShaderClass::NormalMapShaderClass() { m_OpenGLPtr = 0; }
...
bool NormalMapShaderClass::Initialize(OpenGLClass *OpenGL) {
  ...
  strcpy(vsFilename, "./shaders/normalmap.vs");
  strcpy(psFilename, "./shaders/normalmap.ps");
...
bool NormalMapShaderClass::InitializeShader(char *vsFilename,
                                            char *fsFilename) {
  ...
  if (!vertexShaderBuffer) {
    printf("ERROR: Failed to initialize vertexshader at '%s'\n", vsFilename);
    return false;
  }
  ...
  if (!fragmentShaderBuffer) {
    printf("ERROR: Failed to initialize fragmentshader at '%s'\n", fsFilename);
    return false;
  }
...
void NormalMapShaderClass::OutputShaderErrorMessage(unsigned int shaderId,
                                                    char *shaderFilename) {
  long count;
  int logSize, error;
  char *infoLog;
...
bool NormalMapShaderClass::SetShaderParameters(float *worldMatrix,
                                               float *viewMatrix,
