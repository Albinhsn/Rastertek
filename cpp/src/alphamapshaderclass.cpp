...
AlphaMapShaderClass::AlphaMapShaderClass() { m_OpenGLPtr = 0; }
...
bool AlphaMapShaderClass::Initialize(OpenGLClass *OpenGL) {
  ...
  strcpy(vsFilename, "./shaders/alphamap.vs");
  strcpy(psFilename, "./shaders/alphamap.ps");

  result = InitializeShader(vsFilename, psFilename);
  if (!result) {
    printf("ERROR: Failed to initialize shader\n");
    return false;
  }
  return true;
}
...
bool AlphaMapShaderClass::InitializeShader(char *vsFilename, char *fsFilename) {
  ...
  if (!vertexShaderBuffer) {
    printf("Failed to load shader file at '%s'\n", vsFilename);
    return false;
  }
...
void AlphaMapShaderClass::OutputShaderErrorMessage(unsigned int shaderId,
                                                   char *shaderFilename) {
  long count;
  int logSize, error;
  char *infoLog;
...
bool AlphaMapShaderClass::SetShaderParameters(float *worldMatrix,
                                              float *viewMatrix,
  ...
  location =
      m_OpenGLPtr->glGetUniformLocation(m_shaderProgram, "shaderTexture3");
  if (location == -1) {
    return false;
  }
